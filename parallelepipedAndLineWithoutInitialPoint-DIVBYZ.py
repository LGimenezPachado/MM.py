...
def line(m, n, o, p, q):
    if q == 0:
        print("Division by zero in function 'line'")
        return
    ...
    N0 = math.floor(Z)
    if N0 == 0:
        print("Division by zero in function 'line' due to N0 being zero")
        return
    pX = (o - m) / N0
    pY = (p - n) / N0
...
def loop(a, b, c, d, e, f, g, h, i, j, k, l, m, n):
    if g == 0:
        print("Division by zero in function 'loop' due to g being zero")
        return
...
def main():
    ...
    if p >= 0:
        ...
        D1 = math.sqrt(c + d)
        if p == 0:
            print("Division by zero in main due to p being zero")
            return
        Z1 = D1 / p
        N1 = math.floor(Z1) + 1

        if N1 == 1:
            print("Division by zero in main due to N1 being one (N1 - 1 = 0)")
            return
        dX1 = (X1 - X0) / (N1 - 1)
        dY1 = (Y1 - Y0) / (N1 - 1)
        ...
        if N2 == 1:
            print("Division by zero in main due to N2 being one (N2 - 1 = 0)")
            return
        dX2 = (X2 - X0) / (N2 - 1)
        dY2 = (Y2 - Y0) / (N2 - 1)
